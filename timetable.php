<?php
/*
Plugin Name: Kursverwaltung für den Dachsbau
...
<body>
	<div>Danke für deine Buchung</div>
	<div>Nutzer Informationen/div>
	<div><b>Name</b>: {user_name}</div>
	<div><b>Mail</b>: {user_email}</div>
	<div><b>Zeitraum</b>: {booking_datetime}</div>
	<div>Buchungs Informationen</div>
	<div><b>Kurs</b>: {event_title}</div>
	<div><b>Tag</b>: {column_title}</div>
	<div><b>Zeitraum</b>: {event_start} - {event_end}</div>
	<div><b>Platznummer</b>: {slots_number}</div>
...
<body>
	<div>Neue Buchung ist eingegangen</div>
	<div>Mitglied Informationen/div>
	<div><b>Name</b>: {user_name}</div>
	<div><b>Mail</b>: {user_email}</div>
	<div><b>Zeitraum</b>: {booking_datetime}</div>
	<div>Buchungs Informationen</div>
...
<p>Selected event hour doesn't exist.<br>Please select different event.</p>", "timetable"), array("h2" => array(), "p" => array(), "br" => array()));
		...
		$booking_form_config = array(
			...
			'guest_message_field_required' => 'yes',
...
<p>You\'ve already reached maximum number of slots.</p>', 'timetable'), array("h2" => array(), "p" => array()));
		...
		{
			$result['msg'] = wp_kses(__('<h2>Booking couldn\'t be made</h2>
<p>Du hast zu viele Plätze gewählt.</p>', 'timetable'), array("h2" => array(), "p" => array()));
			$result['error'] = 1;
			timetable_ajax_response($result);
		}
		...
		if(!$user_id && $guest_config['allow_guest_booking']=='yes')
		{
			if($guest_phone=='')
			{
				$result['msg'] = wp_kses(__('<h2>Achtung</h2>
<p>Du musst bitte das Vorname Feld ausfüllen.</p>', 'timetable'), array("h2" => array(), "p" => array()));
				$result['error'] = 1;
				timetable_ajax_response($result);
			}
			if($guest_name=='')
			{
				$result['msg'] = wp_kses(__('<h2>Achtung</h2>
<p>Du musst bitte das Nachname Feld ausfüllen.</p>', 'timetable'), array("h2" => array(), "p" => array()));
				$result['error'] = 1;
				timetable_ajax_response($result);
			}
			if($guest_email=='')
			{
				$result['msg'] = wp_kses(__('<h2>Achtung</h2>
<p>Du musst bitte das eMail Feld ausfüllen.</p>', 'timetable'), array("h2" => array(), "p" => array()));
				$result['error'] = 1;
				timetable_ajax_response($result);
			}
			...
			{
				$result['msg'] = wp_kses(__('<h2>Achtung</h2>
<p>Du musst deine eMail validieren</p>', 'timetable'), array("h2" => array(), "p" => array()));
				$result['error'] = 1;
				timetable_ajax_response($result);
			}
			...
			{
				$result['msg'] = wp_kses(__('<h2>Achtung</h2>
<p>Du musst bitte das 2G Feld ausfüllen.</p>', 'timetable'), array("h2" => array(), "p" => array()));
				$result['error'] = 1;
				timetable_ajax_response($result);
			}
			...
			{
				$result['msg'] = wp_kses(__('<h2>Achtung</h2>
<p>Du musst bitte das Mitgliedsnummer Feld ausfüllen.</p>', 'timetable'), array("h2" => array(), "p" => array()));
				$result['error'] = 1;
				timetable_ajax_response($result);
			}
			...
			$member = MEMBERS_CHECK::checkMeberByNumber($guest_message);
			if($member == 0){
				$result['msg'] = wp_kses(__('<h2>Achtung</h2>
				<p>Gib bitte eine gültige Mitgliedsnummer ein. (NUR <u>EINE</u> MITGLIEDNUMMER ERLAUBT) </p>', 'timetable'), array("h2" => array(), "p" => array()));
				$result['error'] = 1;
				timetable_ajax_response($result);
			}
			//
...
function tt_booking_form($args)
{
	...
	if($args['allow_guest_booking']=='yes')
	{
    	...
    	{	
    		$placeholder = ($args['show_guest_phone_field']=='yes' ? esc_html__('Vorname *', 'timetable') : esc_html__('Vorname *', 'timetable'));
    		$output .= 
    		'<div class="tt_field_wrapper ">
    			<label for="tt_guest_phone">' . $placeholder . '</label>
    			<input id="tt_guest_phone" class="tt_field tt_guest_phone" name="Vorname" type="text"  value="" autocomplete="off"/>
    		</div>';
    	}
		if($args['show_guest_name_field']=='yes')
    	{
    		$placeholder = ($args['guest_name_field_required']=='yes' ? esc_html__('Nachname *', 'timetable') : esc_html__('Nachname', 'timetable'));
    		$output .= 
    		'<div class="tt_field_wrapper ">
